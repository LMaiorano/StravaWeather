...
class GoogleMapPlotter(object):
    ...
    def heatmap(self, lats, lngs, weights, threshold=10, radius=10, gradient=None, opacity=0.6, maxIntensity=1, dissipating=True):
        """
        ...
        heatmap_points = []
        for lat, lng, wgt in zip(lats, lngs, weights):
            heatmap_points.append((lat, lng, wgt))
    ...
    def _process_heatmap_kwargs(self, settings_dict):
        ...
        if gradient:
            gradient_string = "var gradient = " + str(gradient) + '\n'
            # for color_step in gradient:
            #     gradient_string += "\t" + color_step+ ",\n"
    ...
    def draw(self, htmlfile, mapType='roadmap', weatherType='Sunny Test'):
        """Create the html file which include one google map and all points and paths. If 
        no string is provided, return the raw html.
        :param mapType: string, one of ('roadmap', 'satellite', 'hybrid', 'terrain')
        """
        f = open(htmlfile, 'w')
        f.write('<html>\n')
        f.write('<head>\n')
        ...
        f.write(f'<title>StravaWeather - {weatherType} </title>\n')
        if self.apikey:
        ...
        self.write_map(f, map_type=mapType)
        self.write_grids(f)
        self.write_points(f)
        self.write_paths(f)
    ...
    def write_map(self,  f, map_type='roadmap'):
        f.write('\t\tvar centerlatlng = new google.maps.LatLng(%f, %f);\n' %
                (self.center[0], self.center[1]))
        f.write('\t\tvar myOptions = {\n')
        f.write('\t\t\tzoom: %d,\n' % (self.zoom))
        f.write('\t\t\tcenter: centerlatlng,\n')
        f.write(f'\t\t\tmapTypeId: "{map_type}"\n')
        f.write('\t\t};\n')
        f.write(
    ...
    def write_heatmap(self, f):
        for heatmap_points, settings_string in self.heatmap_points:
            ...
            for heatmap_lat, heatmap_lng, heatmap_weight in heatmap_points:
                f.write('{location: new google.maps.LatLng(%f, %f), weight: %s},\n' %
                        (heatmap_lat, heatmap_lng, int(heatmap_weight)))
            f.write('];\n')
            f.write('\n')
